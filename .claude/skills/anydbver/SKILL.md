...
- **A coroot-monitored database can be absent from the overview list.** Coroot hides *standalone* applications, ones with no observed traffic to any other application. A single database nobody has connected to is hidden even though it is registered and scraped. The deploy prints a direct `/p/<proj>/applications/<url-encoded-app-id>` link per database — the colons must be percent-encoded or coroot answers `invalid application id`. Replica sets are unaffected.
