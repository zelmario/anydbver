...
- **F2. Monitoring with Coroot** — open-source PMM alternative, `coroot` + `coroot-client:server=<node>`. → [`references/examples.md#coroot`](references/examples.md#coroot)
- **G. Operators on K3D** — k8s-ps, k8s-pxc, k8s-pg (incl. standby clusters), k8s-cnpg (CloudNativePG), k8s-crunchy (Crunchy PGO), k8s-psmdb, k8s-minio. → [`references/topologies.md#operators`](references/topologies.md#operators)
...
- **`coroot` needs no `,docker-image`.** It selects docker-image mode itself; writing it is harmless but unnecessary. It creates five containers (UI + `-clickhouse` `-prometheus` `-cluster-agent` `-node-agent`), all removed by `destroy`. UI login is `admin` / `admin`, not `ANYDBVER_DEFAULT_PASSWORD`.
- **`coroot-client` registration is not instant.** It runs after the playbook and waits for coroot to discover the container, typically one to two minutes. The deploy prints what it is waiting for; that is not a hang.
- **A coroot-monitored database takes a couple of minutes to appear in the application list.** Coroot hides *standalone* applications, ones with no observed traffic to anything else, so a database shows up only once coroot links it to the cluster-agent scraping it. anydbver restarts the node-agent a second time after the cluster-agent starts scraping so that connection gets recorded; coroot then needs a few refresh cycles. Don't call it broken before ~3 minutes. The deploy prints a direct `/p/<proj>/applications/<url-encoded-app-id>` link per database that works immediately — the colons must be percent-encoded or coroot answers `invalid application id`.
- **Coroot eBPF is degraded on Docker Desktop / WSL2.** The node-agent only resolves containers and processes that existed when it started, because eBPF reports root-pid-namespace pids and every WSL2 distro is nested. Database metrics, node inventory and the DB-to-DB connection map still work; live query capture from clients started later does not. Do not try to fix this locally, it needs a real Linux host.
