...
func ParseDeploymentKeyword(logger *log.Logger, keyword string) DeploymentKeywordData {
	...
	if deployCmd == "mongos-shard" || deployCmd == "mongos-cfg" || deployCmd == "haproxy-pg" || deployCmd == "pgbouncer" {
...
func deployHost(provider string, logger *log.Logger, namespace string, name string, ansible_hosts_run_file string, args []string) {
	if provider == "docker-image" {
		...
		for {
			re_mongo_shard_hosts := regexp.MustCompile(`(extra_mongos_shard|extra_mongos_cfg|extra_haproxy_pg|extra_pgbouncer|extra_patroni_standby)='([^']*)(node[0-9]+)([^']*)'`)
