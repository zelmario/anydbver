...
echo "Waiting for PostgreSQL to be ready via Patroni..."
for i in {1..60}; do
...
if [[ "$IS_LEADER" == "leader" ]]; then
    ...
    for i in {1..30}; do
        if sudo -u postgres psql -h localhost -tAc "SELECT 1 FROM pg_roles WHERE rolname='$PMM_USER'" | grep -q 1; then
            echo "PMM user found"
            break
        fi
        sleep 2
