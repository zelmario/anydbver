...
EOF
    ...
    unset PGHOSTADDR
    # Publisher (master): create the logical database, role and publication.
    PGPASSWORD="$MASTER_PASSWORD" PGHOST=$MASTER_IP PGDATABASE=$MASTER_DB PGUSER=$MASTER_USER psql <<EOF
SELECT 'CREATE USER sbtest WITH PASSWORD ''$MASTER_PASSWORD''' WHERE NOT EXISTS (SELECT FROM pg_roles WHERE rolname='sbtest')\gexec
SELECT 'CREATE DATABASE $LOGICAL_DB' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname='$LOGICAL_DB')\gexec
...
SELECT 'CREATE PUBLICATION ${LOGICAL_DB}_publication FOR ALL TABLES' WHERE NOT EXISTS (SELECT FROM pg_publication WHERE pubname='${LOGICAL_DB}_publication')\gexec
EOF
    ...
    # Subscriber (local): create the matching database, load schema and subscribe.
